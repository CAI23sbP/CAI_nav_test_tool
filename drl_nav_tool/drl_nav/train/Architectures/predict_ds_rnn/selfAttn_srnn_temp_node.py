...
from .predict_srnn_model import *
...
class SpatialEdgeSelfAttn(nn.Module):
    ...
    def forward(self, inp, each_seq_len):
        ...
        each_seq_len:
        if self.config.sort_humans is True, the true length of the sequence. Should be the number of detected humans
        else, it is the mask itself
        '''
        # inp is padded sequence [seq_len, nenv, max_human_num, 2]
        seq_len, nenv, max_human_num, _ = inp.size()
        if self.config.sort_humans:
...
class selfAttn_merge_SRNN(nn.Module):
    ...
    def __init__(self, obs_space_dict, config, infer=False):
        ...
        robot_size = 6
        self.robot_linear = nn.Sequential(init_(nn.Linear(robot_size, 256)), nn.ReLU()).to("cuda:0") # todo: check dim
        self.human_node_final_linear=init_(nn.Linear(self.output_size,2))

        if self.config.use_self_attn:
    ...
    def forward(self, inputs, rnn_hxs, masks, infer=False):
        ...
        if not hasattr(self.config, 'sort_humans'):
            self.config.sort_humans = True
        if self.config.sort_humans:
        ...
        robot_states = self.robot_linear(robot_states.cuda()) ## TODO transform to GPU
