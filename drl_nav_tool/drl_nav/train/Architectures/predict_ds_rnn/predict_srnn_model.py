...
class RNNBase(nn.Module):
    ...
    def _forward_gru(self, x, hxs, masks):
        ...
        # during update, input shape[0] * nsteps (30) = hidden state shape[0]
            ...
            has_zeros = (torch.nonzero((masks[1:] == 0.0) \
                            .any(dim=-1)
                            .squeeze()
                            .cpu()))
            ...
            for i in range(len(has_zeros) - 1):
                ...
                if type(end_idx) == type(list()) and type(start_idx) != type(list()):
                    x_in = x[start_idx:end_idx[0]]

                elif type(start_idx) == type(list()) and type(end_idx) != type(list()):
                    x_in = x[start_idx[0]:end_idx]

                elif type(end_idx) == type(list()) and type(start_idx)==type(list()):
                    x_in = x[start_idx[0]:end_idx[0]]
...
class SRNN(nn.Module):
    ...
    def __init__(self, obs_space_dict, config, infer=False):
        ...
        self.nenv = config.num_processes
        self.nminibatch = config.num_mini_batch
        ...
        spatial_config = copy.deepcopy(config)
        spatial_config.human_human_edge_input_size = obs_space_dict['spatial_edges'].shape[1]
        self.humanhumanEdgeRNN_spatial = HumanHumanEdgeRNN(spatial_config)
