...
class GumbelSocialTransformer(nn.Module):
    def __init__(self, d_motion, d_model, nhead_nodes, nhead_edges, nlayer, dim_feedforward=512, dim_hidden=32, \
        ...
        if ghost:
            ...
            print("Ghost version.")
            from .edge_ghost import EdgeSelector
            from .node_ghost import NodeEncoderLayer
        else:
            print("No ghost version.")
            from .edge_no_ghost import EdgeSelector
            from .node_no_ghost import NodeEncoderLayer
        if nhead_edges != 0:
