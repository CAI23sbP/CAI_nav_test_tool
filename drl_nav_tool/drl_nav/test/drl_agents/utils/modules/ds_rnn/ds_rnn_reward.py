...
class DSRNNRewardCalculator:
    ...
    def transform_robot_frame(robot_node ):
        ...
        radius = robot_node[:,:,2].item()  ## robot raidus
    ...
    def get_reward(self,obs_dict):
        self._reset()
        dist_to_human =self._cal_dist_to_human(obs_dict["spatial_edges"].cpu())  
    ...
    def _cal_reward_rule(self,dist_to_human,goal_in_robot_frame):
        ...
        self._reward_safe_dist(dist_to_human, punishment=2.5)
    ...
    def _reward_goal_approached(
        ...
        **kwargs,
        ...
        """
        # [distance_goal, cx,cy ,px, py, radius, gx ,gy]
    ...
    def _reward_collision(
        ...
        if len(dist) != 0:
            if np.array(dist).min() <= goal_in_robot_frame[3]+0.6: # 0.6 is human radius
