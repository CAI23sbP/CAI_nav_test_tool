...
class AdDWAObservationCollector:
    ...
    def __init__(self,max_human_num,scan_dim):
        ...
        self.scan_dist = np.ones(scan_dim)*3.5
        self.scan_pose = np.ones((scan_dim,2))*3.5
        # sub
    ...
    def wait_for_scan_and_odom(self):
        while not self.received_odom and not self.received_scan and not self.received_human:
            pass
    ...
    def get_observations(self, *args, **kwargs):
        ...
        rho, theta = AdDWAObservationCollector._get_goal_pose_in_robot_frame(
        ...
        observation = {
            ...
            'scan_dist':np.array(self.scan_dist),
            'scan_pose':np.array(self.scan_pose),
            }
        
        return observation

    def callback_global_plan(self, msg_global_plan):
        self._globalplan = AdDWAObservationCollector.process_global_plan_msg(
            msg_global_plan
        )
        return
