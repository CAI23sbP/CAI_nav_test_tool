...
class AdDWARewardCalculator:
    ...
    def _cal_reward_rule(self,state,goal_in_robot_frame):
        ...
        self._reward_safe_dist(state, punishment=0.1)
    ...
    def _reward_goal_approached(
        ...
        **kwargs,
        ...
        if len(self.init_pose) > 1:
            ...
            self.init_pose.append([goal_in_robot_frame[7], goal_in_robot_frame[8]])
        max_dist = np.linalg.norm([self.init_pose[0][0]-goal_in_robot_frame[5], self.init_pose[0][1]-goal_in_robot_frame[6]])
    ...
    def _reward_safe_dist(
        self, state: np.ndarray, punishment: float = 0.15, *args, **kwargs
    ):
        """
