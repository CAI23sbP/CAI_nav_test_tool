...
class MYDWAObservationCollector:
    ...
    def scan_range(self,msg):
        ...
        distance = scan_data[np.logical_not(np.isnan(scan_data))]
        ...
        for i in distance:
            ...
            position.append([x,y])
        self.scan_data = np.array(position) 
    ...
    def wait_for_scan_and_odom(self):
        while not self.received_odom or not self.received_scan or not self.received_human:
            pass
    ...
    def get_observations(self, *args, **kwargs):
        ...
        human_state = np.zeros((self.max_human_num,2))
        cluster = self.fill_cluster_dist().mean_points
       
        for i in range(len(cluster)):
            human_state[i] = [cluster [i].x,cluster [i].y]
                
        rho, theta = MYDWAObservationCollector._get_goal_pose_in_robot_frame(
            self._globalplan, self._robot_pose
        )

        robot_node = [rho, theta, 0.3, self.linear, self.anguler, self._globalplan.x, self._globalplan.y, self._robot_pose.x, self._robot_pose.y, self._robot_pose.theta]
        observation = {
            'robot_state': np.array(robot_node),
            'human_pose':np.array(human_state),
            'scan_state':np.array(self.scan_data),
            }
        
        return observation

    def callback_global_plan(self, msg_global_plan):
        self._globalplan = MYDWAObservationCollector.process_global_plan_msg(
            msg_global_plan
        )
        return
