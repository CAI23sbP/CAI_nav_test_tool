...
class Recorder:
    def __init__(self):
        self.take_sencario = Constants.TaskMode.Scenario.YAML_PATH
        self.model = rospy.get_param(os.path.join(rospy.get_namespace(), "model"), "")
        self.get_start_goal = self.get_goal_start_pose()
    ...
    def get_goal_start_pose(self):
        name = self.take_sencario
        with open(name) as file:
            content = yaml.safe_load(file)

        start = content["robot_position"]
        goal = content["robot_goal"]
    ...
    def clock_callback(self, clock: Clock):
        ...
        self.write_data("start_goal", [
            ...
            rospy.get_param(rospy.get_namespace() + "start", self.get_start_goal[0]), 
            rospy.get_param(rospy.get_namespace() + "goal", self.get_start_goal[1])
        ])
    ...
    def write_params(self):
        with open(self.result_dir + "/params.yaml", "w") as file:
            yaml.dump({
                ...
                "scenario_file": f"{self.take_sencario.split('/')[-1]}",
