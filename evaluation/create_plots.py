...
class PathVisualizer:
    ...
    def __init__(self, scenario):
        ...
        self.map_name = self.scenario_content["map_path"]
    ...
    def create_episode_plots_for_namespaces(self, dataset, title, save_name, episode=0, differentiate="local_planner", desired_results=[], should_add_obstacles=False, should_add_collisions=False):
        ...
        for namespace in robots_tested:
            ...
            for i in iterator:
                ...
                # print(len(path.tolist()))      
                # print(pd.DataFrame(path))       
                # df = pd.DataFrame(path).reset_index()["path"]

                # for j in range(len(df)):
                #     # print(df[j])
                #     # print(len(desired_results) > 0 and not result in desired_results)
    ...
    def add_obstacles_to_plot(self, ax, should_add_obstacles=False):
        if not should_add_obstacles:
            return
        # static = self.scenario_content["pedsim_agents"]

        # for o in static["static"]:
        #     position = o["pos"]
    ...
    def read_scenario_file(scenario):
        ...
        name = os.path.join(rospkg.RosPack().get_path("task_generator"), "scenario", scenario)
    ...
    def read_map_file(map_name):
        map_path = os.path.join(rospkg.RosPack().get_path("robohumstamap"), "map_type", map_name)
